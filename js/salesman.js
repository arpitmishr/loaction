...
import { appCache, getCachedUserProfile } from "./auth.js"; 
...
function startGeoFencing(targetLat, targetLng, outletId, outletName) {
    ...
    watchId = navigator.geolocation.watchPosition((pos) => {
        ...
        if (distMeters <= GEO_FENCE_RADIUS) {
            ...
            checkInBtn.style.background = "#6c757d"; 
        }
    }, (err) => {
...
window.submitLeaveRequest = async function() {
    ...
    try {
        await addDoc(collection(db, "leaves"), {
            salesmanId: auth.currentUser.uid,
            salesmanEmail: auth.currentUser.email, 
            date: date, 
            type: type,
            reason: reason,
            status: "pending",
